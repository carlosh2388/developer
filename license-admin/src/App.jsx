  ...
  async function submit(event) {
    ...
    try {
      const data = await api("/auth/login", {
        method: "POST",
        body: JSON.stringify({ ...form, userScope: "PLATFORM" }),
      });
          ...
          <div className="form-grid">
            ...
            <label>
              ...
              <input type="number" min="1" max="10000" value={form.maxUsers}
                onChange={(e) => setForm({ ...form, maxUsers: Number(e.target.value) })} required />
            </label>
      ...
      {editing && (
        ...
        >
          <form onSubmit={saveEdit}>
            ...
            <label>
              Usuarios permitidos
              <input type="number" min="1" max="10000" value={editing.maxUsers}
                onChange={(e) => setEditing({ ...editing, maxUsers: Number(e.target.value) })} required />
            </label>
            <label>
              Estado
              <select
...
function Dashboard({ user, onLogout }) {
  ...
  const titles = {
    ...
    users: ["Usuarios", "Gestiona usuarios del control central y de cada cliente."],
    ...
    <div className="layout">
      <aside>
        ...
        <nav>
          ...
          <button
            ...
            onClick={() => { setView("users"); setNotice(null); }}
          >
            Usuarios
          </button>
          <button
      ...
      <main className="content">
        ...
        {loading ? (
          ...
          <UsersView organizations={organizations} notify={notify} />
        ) : (
          <AuditView
            organizations={organizations}
            logs={logs}

...
export default function ConfigRecordsTable({ title, rows, columns, loading, error, dateField, onEdit, onDeactivate, deactivateLabel = "Dar de baja", inactiveStatuses = ["INACTIVE"], buttonStyle }) {
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    if (query && !columns.some((column) => text(row[column.key]).toLowerCase().includes(query))) return false;
    if (status && row.status !== status) return false;
    const date = dateField ? text(row[dateField]).slice(0, 10) : "";
    if (from && date && date < from) return false;
    if (to && date && date > to) return false;
    return columns.every((column) => !filters[column.key] || text(row[column.key]).toLowerCase().includes(filters[column.key].toLowerCase()));
  ...
  return <>
    ...
    <section role="dialog" aria-modal="true" aria-label={title} onKeyDown={(event) => { if (event.key === "Enter") event.preventDefault(); }} style={{ position: "relative", width: "min(1400px, 96vw)", maxHeight: "90vh", overflow: "auto", background: "#fff", borderRadius: 12, padding: 22, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
      ...
      <select value={status} onChange={(e) => setStatus(e.target.value)} style={control}><option value="">Todos los estados</option><option value="ACTIVE">Activos</option><option value="INACTIVE">Inactivos</option></select>
    ...
    {error && <p style={{ color: "#b42318" }}>{error}</p>}{loading ? <p>Cargando registros…</p> : <div style={{ overflowX: "auto" }}>
      ...
      <tbody>{filtered.map((row) => <tr key={row.id}>{columns.map((column) => <td key={column.key} style={{ padding: 8, borderBottom: "1px solid #eee" }}>{column.render ? column.render(row[column.key], row) : text(row[column.key])}</td>)}{(onEdit || onDeactivate) && <td style={{ whiteSpace: "nowrap" }}>{onEdit && <button type="button" onClick={() => { onEdit(row); setOpen(false); }} style={{ padding: "6px 10px", border: 0, borderRadius: 5, background: "#1976d2", color: "#fff", cursor: "pointer", fontWeight: 600 }}>Editar</button>} {onDeactivate && !inactiveStatuses.includes(row.status) && <button type="button" onClick={() => onDeactivate(row)} style={{ padding: "6px 10px", border: 0, borderRadius: 5, background: "#c62828", color: "#fff", cursor: "pointer", fontWeight: 700 }}>{deactivateLabel}</button>}</td>}</tr>)}</tbody></table>

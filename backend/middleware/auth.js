  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT u.id, u.organization_id, u.full_name, u.username, u.status, u.token_version,
              u.is_platform_admin, r.code AS role,
              COALESCE(array_agg(p.code) FILTER (WHERE p.code IS NOT NULL), '{}') AS permissions
       FROM users u JOIN roles r ON r.id = u.role_id
       LEFT JOIN role_permissions rp ON rp.role_id = r.id
       LEFT JOIN permissions p ON p.id = rp.permission_id
       WHERE u.id = $1 GROUP BY u.id, r.code`,
      [payload.sub]
    );
    const user = rows[0];
    if (!user || user.status !== "ACTIVE" || user.token_version !== payload.ver) {
      throw new HttpError(401, "Tu sesión ya no es válida. Inicia sesión nuevamente.", "SESSION_REVOKED");
    }
    req.user = {
      id: user.id, organizationId: user.organization_id, name: user.full_name,
...
const requirePlatformAdmin = (req, _res, next) => {
  if (!req.user.isPlatformAdmin) return next(new HttpError(403, "Esta función es exclusiva del administrador de la plataforma.", "PLATFORM_ADMIN_REQUIRED"));
  next();
};

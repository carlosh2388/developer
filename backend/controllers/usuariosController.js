...
async function obtenerUsuarios(req, res, next) {
  try {
    const { rows } = await db.query(
      `SELECT ${publicFields} FROM users u JOIN roles r ON r.id = u.role_id
       WHERE u.organization_id = $1 ORDER BY u.full_name`, [req.user.organizationId]
    );
    res.json(rows);
...
async function crearUsuario(req, res, next) {
  ...
  try {
    const { fullName, username, password, roleCode } = req.body;
    ...
    const { rows } = await client.query(
      `INSERT INTO users (organization_id, role_id, full_name, username, password_hash, created_by)
       VALUES ($1, $2, $3, LOWER($4), $5, $6) RETURNING id`,
...
async function actualizarUsuario(req, res, next) {
  try {
    const { fullName, username, roleCode, status } = req.body;
    if (!fullName || !username || !roleCode || !["ACTIVE", "INACTIVE", "LOCKED"].includes(status)) {
      throw new HttpError(400, "Los datos enviados no son válidos.", "VALIDATION_ERROR");
    }
    ...
    const { rowCount } = await db.query(
      `UPDATE users u SET full_name=$1, username=LOWER($2), status=$3, token_version=token_version+1,
              role_id=(SELECT id FROM roles WHERE code=$4), updated_at=NOW()
       WHERE u.id=$5 AND u.organization_id=$6`,
      [fullName.trim(), username.trim(), status, roleCode, req.params.id, req.user.organizationId]
    );
    if (!rowCount) throw new HttpError(404, "Usuario no encontrado.", "USER_NOT_FOUND");
...
async function cambiarEstado(req, res, next) {
  try {
    ...
    const result = await db.query(
      "UPDATE users SET status=$1, token_version=token_version+1, updated_at=NOW() WHERE id=$2 AND organization_id=$3",
    ...
    res.json({ message: status === "ACTIVE" ? "Acceso del usuario reactivado." : "El usuario fue dado de baja y sus sesiones fueron revocadas." });
  } catch (error) { next(error); }
}
...
async function restablecerPassword(req, res, next) {
  try {
    ...
    const result = await db.query(
      "UPDATE users SET password_hash=$1, token_version=token_version+1, updated_at=NOW() WHERE id=$2 AND organization_id=$3",
...
module.exports = { obtenerUsuarios, obtenerRoles, crearUsuario, actualizarUsuario, cambiarEstado, restablecerPassword };

  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT l.id,l.organization_id,o.name AS organization_name,l.installation_id,l.status,l.starts_at,l.expires_at,
              l.grace_days,l.max_users,l.last_check_at,l.notes,l.created_at,l.updated_at
...
async function createLicense(req, res, next) {
  ...
  try {
    const { organizationId, installationId, expiresAt, graceDays = 0, maxUsers, notes } = req.body;
    ...
    if (!Number.isInteger(Number(maxUsers)) || Number(maxUsers) < 1 || Number(maxUsers) > 10000) {
      throw new HttpError(400, "La cantidad de usuarios debe estar entre 1 y 10000.", "INVALID_MAX_USERS");
    }
    ...
    const { rows } = await client.query(
      `INSERT INTO licenses(organization_id,installation_id,license_key_hash,expires_at,grace_days,max_users,notes)
       VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id,status,expires_at,max_users`,
      [organizationId, installationId.trim(), hash, expiresAt, graceDays, Number(maxUsers), notes || null]
    );
    await client.query(
...
async function updateLicense(req, res, next) {
  try {
    const { status, expiresAt, graceDays, maxUsers, notes } = req.body;
    ...
    if (maxUsers != null && (!Number.isInteger(Number(maxUsers)) || Number(maxUsers) < 1 || Number(maxUsers) > 10000)) {
      throw new HttpError(400, "La cantidad de usuarios debe estar entre 1 y 10000.", "INVALID_MAX_USERS");
    }
    if (maxUsers != null) {
      const usage = await db.query(
        `SELECT COUNT(u.id) FILTER (WHERE u.status='ACTIVE')::int AS active_users
         FROM licenses l LEFT JOIN users u ON u.organization_id=l.organization_id AND u.user_scope='CLIENT'
         WHERE l.id=$1 GROUP BY l.id`, [req.params.id]
      );
      if (usage.rows[0] && usage.rows[0].active_users > Number(maxUsers)) {
        throw new HttpError(409, `No puedes reducir el cupo a ${maxUsers}; el cliente tiene ${usage.rows[0].active_users} usuario(s) activos.`, "MAX_USERS_BELOW_USAGE");
      }
    }
    const { rows } = await db.query(
      `UPDATE licenses SET status=$1,expires_at=COALESCE($2,expires_at),grace_days=COALESCE($3,grace_days),
              max_users=COALESCE($4,max_users),notes=$5,updated_at=NOW() WHERE id=$6
       RETURNING id,status,expires_at,grace_days,max_users,notes`,
      [status, expiresAt || null, graceDays ?? null, maxUsers ?? null, notes || null, req.params.id]
    );
    ...
    await audit(req, action, "licenses", req.params.id, { status, expiresAt, graceDays, maxUsers });
